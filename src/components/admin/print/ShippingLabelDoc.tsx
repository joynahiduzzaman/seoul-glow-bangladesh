...
/**
 * A physical parcel label — no prices beyond the COD amount the courier has to
 * collect (same reasoning as the packing slip). Renders even when no shipment
 * has been assigned, so a label shell can be printed before a courier is
 * picked; the courier-specific fields just show as blanks.
 *
 * Shared by the single-order and batch print routes.
 */
export default function ShippingLabelDoc({
  order,
  shipment,
  siteName,
}: {
  order: PrintableOrder & { insideDhaka: boolean };
  shipment: LabelShipment | null;
  siteName: string;
}) {
