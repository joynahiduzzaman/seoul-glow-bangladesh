...
/**
 * The invoice document itself, without the page chrome.
 *
 * Extracted so the single-order print route and the batch route render the
 * identical document — a second copy of this markup would drift, and an invoice
 * that differs depending on how it was printed is a real problem.
 */
    ...
    <div className="bg-white border border-ink/10 rounded-xl2 p-10 shadow-soft print:shadow-none print:border-0 print:p-0">
      <div className="flex items-start justify-between mb-10 pb-6 border-b border-ink/10">
        <div>
          ...
          {supportEmail && <p className="text-xs text-ink/70 mt-1">{supportEmail}</p>}
        </div>
